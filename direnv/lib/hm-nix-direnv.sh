...
NIX_DIRENV_FALLBACK_NIX=${NIX_DIRENV_FALLBACK_NIX:-/nix/store/fazlzakydlgbfylk82ym7si9n4hdpbqs-nix-2.31.2/bin/nix}
...
_require_version() {
  ...
  if ! printf "%s\n" "$required" "$version" | LC_ALL=C /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/sort -c -V 2>/dev/null; then
    _nix_direnv_error \
      "minimum required $(/nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/basename "$cmd") version is $required (installed: $version)"
    return 1
  fi
}
...
_nix_direnv_preflight() {
  ...
  if [[ ! -d "$layout_dir/bin" ]]; then
    /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/mkdir -p "$layout_dir/bin"
  fi
  ...
  /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/cat >"${layout_dir}/bin/nix-direnv-reload" <<-EOF
#!/usr/bin/env bash
set -e
if [[ ! -d "$PWD" ]]; then
...
EOF
  ...
  /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/chmod +x "${layout_dir}/bin/nix-direnv-reload"
...
_nix_import_env() {
  ...
  if [[ -n ${NIX_BUILD_TOP+x} && $NIX_BUILD_TOP == */nix-shell.* && -d $NIX_BUILD_TOP ]]; then
    /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/rm -rf "$NIX_BUILD_TOP"
  fi
...
_nix_clean_old_gcroots() {
  ...
  /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/rm -rf "$layout_dir/flake-inputs/"
  /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/rm -f "$layout_dir"/{nix,flake}-profile*
}
...
_nix_argsum_suffix() {
  ...
  if [ -n "$1" ]; then
    ...
    if has sha1sum; then
      out=$(/nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/sha1sum <<<"$1")
  ...
  if [[ ! -e $profile ||
    ...
    if [[ $_nix_direnv_manual_reload -eq 1 && -z ${_nix_direnv_force_reload-} ]]; then
      ...
      if tmp_profile_rc=$(_nix print-dev-env --profile "$tmp_profile" "$@"); then
        ...
        /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/rm -f "$tmp_profile" "$tmp_profile"*

        # also add garbage collection root for source
        local flake_input_paths
        /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/mkdir -p "$flake_inputs"
  ...
  if path=$(_nix eval --impure --expr "<nixpkgs>" 2>/dev/null); then
    if [[ -f "${path}/.version-suffix" ]]; then
      ...
      read -r version_prefix < <(
        /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/cat "${path}/.version"
        echo
      )
...
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/basename
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/cat
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/chmod
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/mkdir
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/rm
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/sha1sum
# resholve: keep /nix/store/jbz6j4iwnrvki1zl34hwcyj2i0m6l2y1-coreutils-9.8/bin/sort
